...
export async function action({ request }: ActionFunctionArgs) {
	...
	switch (method) {
		...
		case "delete": {
			if (!data.has("id")) {
				return send({ message: "No id provided" }, 400);
			}

			const username = String(data.get("username"));
			const id = String(data.get("id"));
			await del(`v1/user/${id}`, session.get("hsApiKey")!);
			...
			>
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						<Rename username={user.name} id={user.id} magic={magic} />
						{user.machines.length === 0 ? (
							<Remove username={user.name} id={user.id} />
						) : undefined}
					</div>
				</div>
				<div className="mt-4">
