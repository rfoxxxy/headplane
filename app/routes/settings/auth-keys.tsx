	...
	const filteredKeys = keys.filter((key) => {
		...
		if (status !== "All") {
			...
			if (status === "Used/Expired") {
				return (key.used && !key.reusable) || expiry < now;
			}
			...
			<div className="flex justify-between gap-4 mt-4">
				<div className="w-full">
					...
					>
						...
						{users.map((user) => (
							<Select.Item key={user.id} id={user.email || user.name}>
								{user.name}
							</Select.Item>
						))}
					</Select>
				...
				<div className="w-full">
					...
					>
						...
						<Select.Item id="Used/Expired">Inactive</Select.Item>
				...
				{filteredKeys.length === 0 ? (
					...
					filteredKeys.map((key) => {
						const matchedUser = users.find(
							(user) => user.email === key.user || user.name === key.user,
						);
